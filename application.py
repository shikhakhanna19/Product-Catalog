...
def showCatalog():
    ...
    if 'username' not in login_session:
        return render_template('publiccatalog.html', catalogs=categories)
    else:
        return render_template('categories.html', catalogs=categories)
...
def newCatalog():
    ...
    if request.method == 'POST':
        ...
        flash('New Restaurant %s Successfully Created' % newCatalog.name)
        session.commit()
...
def editCatalog(catalog_id):
    ...
    if editedCatalog.user_id != login_session['user_id']:
        return (
                ...
                "your own Catalog in order to edit.');}</script>" +
...
def deleteCatalog(catalog_id):
    ...
    if catalogToDelete.user_id != login_session['user_id']:
        return (
                ...
                "your own catalog in order to delete.');}</script>" +
                "<body onload='myFunction()'>")
    if request.method == 'POST':
        session.delete(catalogToDelete)
        session.commit()
        return redirect(
            url_for('showCatalog', catalog_id=catalog_id))
    else:
        return render_template(
...
def newcatagoryItem(catalog_id):
    if 'username' not in login_session:
        return redirect('/login')
    ...
    if login_session['user_id'] != catalog.user_id:
        return (
                ...
                "your own catalog in order to add items.');}</script>" +
                "<body onload='myFunction()'>")
    if request.method == 'POST':
        newItem = CatalogItem(
                                name=request.form['name'],
        ...
        return render_template('newCatlogItem.html', catalog_id=catalog_id)
...
def editCatalogItem(catalog_id, menu_id):
    ...
    if login_session['user_id'] != catalog.user_id:
        return (
            ...
            "edit menu items to this catalog.Please create your own catalog" +
...
def deleteCatalogItem(catalog_id, menu_id):
    ...
    if login_session['user_id'] != catalog.user_id:
        return (
                "<script>function myFunction(){alert('You are not authorized" +
                "to delete menu items to this catalog.Please create your own" +
                "catalog in order to delete items.');}</script>" +
                "<body onload='myFunction()'>")
    if request.method == 'POST':
